...
interface Pokemon {
  ...
  MYTHIC?: string
  LEGENDARY?: string
  'Previous evolution(s)'?: Array<{ id: number, name: string }>
  ...
  async run (em: EntityManager): Promise<void> {
    ...
    pokemonsMap.forEach((pokemon, key) => {
      ...
      for (const resistance of pokemon.resistant) {
        const type = getOrCreateType(resistance)
      ...
      for (const weakness of pokemon.weaknesses) {
        const type = getOrCreateType(weakness)
        const attribute = new PokemonTypeAttribute()
        wrap(attribute).assign({
          type,
      ...
      if (pokemon['Previous evolution(s)'] != null) {
